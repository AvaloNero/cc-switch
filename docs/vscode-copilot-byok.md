...
- After fetching a provider's model list, CC Switch enriches exact or unambiguous model matches from the same models.dev capability catalog used by OpenCode. Tool calling, vision, reasoning, context, and input/output limits are filled automatically for common model families; endpoint-specific contracts are applied separately when models.dev cannot express a required request constraint. Unknown or ambiguous models remain on VS Code's automatic defaults.
