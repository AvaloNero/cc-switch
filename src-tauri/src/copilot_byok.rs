...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct CopilotProxyCustomTarget {
    id: String,
    name: String,
...
struct CopilotProxyStore {
    ...
    #[serde(default)]
    custom_targets: Vec<CopilotProxyCustomTarget>,
}
...
impl Default for CopilotProxyStore {
    fn default() -> Self {
        Self {
...
impl CopilotProxyStore {
    fn normalize(&mut self) -> Result<(), AppError> {
        ...
        let current_is_available = self.current_provider_id.as_deref().is_some_and(|id| {
            self.providers
                .iter()
...
fn save_store(store: &CopilotProxyStore) -> Result<(), AppError> {
    let path = store_path();
...
fn load_store() -> Result<CopilotProxyStore, AppError> {
    let path = store_path();
    if !path.exists() {
        let store = CopilotProxyStore::default();
        save_store(&store)?;
        return Ok(store);
    }

    let text = fs::read_to_string(&path).map_err(|error| AppError::io(&path, error))?;
    let mut store: CopilotProxyStore = serde_json::from_str(&text).map_err(|error| {
...
fn managed_group(store: &CopilotProxyStore) -> Value {
    let provider = store.current_provider_id.as_deref().and_then(|id| {
        store
            .providers
            .iter()
    ...
    let tool_calling = provider.map(|provider| provider.tool_calling).unwrap_or(true);
    let vision = provider.map(|provider| provider.vision).unwrap_or(false);
    let thinking = provider.map(|provider| provider.thinking).unwrap_or(true);
    let streaming = provider.map(|provider| provider.streaming).unwrap_or(true);
...
fn sync_vscode(store: &CopilotProxyStore) -> Result<CopilotByokSyncResult, AppError> {
    let detected = vscode::discover_vscode_targets()?;
    let available_ids: HashSet<String> = detected
        ...
        .collect();
    let target_ids: Vec<String> = effective_selected_target_ids(store, &detected)
        .into_iter()
...
fn build_state(store: CopilotProxyStore) -> Result<CopilotByokState, AppError> {
    let detected = vscode::discover_vscode_targets()?;
    let available_ids: HashSet<String> = detected
        .iter()
        .map(|target| target.id.clone())
        .chain(store.custom_targets.iter().map(|target| target.id.clone()))
        .collect();
    let selected_target_ids: Vec<String> = effective_selected_target_ids(&store, &detected)
        .into_iter()
        .filter(|id| available_ids.contains(id))
        .collect();
...
async fn proxy_handler(headers: HeaderMap, Json(mut body): Json<Value>) -> Response {
    ...
    let Some(provider_id) = store.current_provider_id.as_deref() else {
        return error_response(StatusCode::SERVICE_UNAVAILABLE, "No proxy provider selected");
    };
    let Some(provider) = store
        ...
        return error_response(
            ...
            "Selected proxy provider is unavailable",
        );
    };
...
async fn stop_server() {
    let runtime = SERVER_RUNTIME.lock().ok().and_then(|mut slot| slot.take());
...
async fn activate_store(store: &CopilotProxyStore) -> Result<CopilotByokSyncResult, AppError> {
...
pub async fn get_state() -> Result<CopilotByokState, AppError> {
    ...
    if store.integration_enabled && store.current_provider_id.is_some() {
        ensure_server(store.listen_port).await?;
        let _ = sync_vscode(&store)?;
    }
    build_state(store)
}
...
pub async fn set_targets(target_ids: Vec<String>) -> Result<CopilotByokState, AppError> {
    let mut store = {
        ...
        let previous: HashSet<String> = effective_selected_target_ids(&store, &detected)
            .into_iter()
...
pub async fn add_custom_target(
    ...
    let mut store = {
        ...
        let target = CopilotProxyCustomTarget {
            id: id.clone(),
            name: name
                .as_deref()
                .map(str::trim)
        ...
        store.custom_targets.push(target);
        store.targets_initialized = true;
        if !store.selected_target_ids.contains(&id) {
...
pub async fn upsert_model(mut model: CopilotByokModel) -> Result<CopilotByokState, AppError> {
    ...
    let store = {
        ...
        if store.current_provider_id.as_deref().is_none_or(|id| {
            !store
                .providers
                .iter()
                .any(|provider| provider.id == id && provider.enabled)
        }) {
            store.current_provider_id = store
                .providers
                .iter()
                .find(|provider| provider.enabled)
                .map(|provider| provider.id.clone());
...
pub async fn delete_model(model_id: &str) -> Result<CopilotByokState, AppError> {
    let store = {
        ...
        if store.current_provider_id.as_deref() == Some(model_id) {
            store.current_provider_id = store
                .providers
                .iter()
                .find(|provider| provider.enabled)
...
pub async fn remove_managed_models(
    ...
    let (store, target_ids, changed_target_count) = {
        ...
        let target_ids = requested_ids
            ...
            .unwrap_or_else(|| effective_selected_target_ids(&store, &detected));
