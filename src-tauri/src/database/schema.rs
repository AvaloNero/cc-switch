...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    16 => {
                        log::info!(
                            "迁移数据库从 v16 到 v17（Skills/MCP 添加 VS Code Copilot 支持）"
                        );
    ...
    /// v16 -> v17: persist VS Code Copilot enablement for unified Skills and MCP.
...
mod tests {
    ...
    #[test]
    fn migrate_v16_to_v17_adds_copilot_enablement_columns() -> Result<(), AppError> {
        let conn = Connection::open_in_memory()?;
        conn.execute_batch(
