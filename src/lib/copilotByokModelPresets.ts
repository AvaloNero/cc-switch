...
function kimiContractPreset(
  ...
  if (!isKimiProvider(context)) return null;
  const family = kimiModelFamily(modelId, modelName, canonicalModelId);
  ...
  if (family === "k3") {
    return {
      ...base,
      contextWindow: /(?:^|[^a-z0-9])256k(?:[^a-z0-9]|$)/i.test(
        `${modelId} ${modelName}`,
      )
        ? 262_144
        : 1_000_000,
...
function miniMaxContractPreset(
  context: CopilotByokModelPresetContext,
  modelId: string,
): CopilotByokModelPreset | null {
  const family = isMiniMaxProvider(context)
 ...
 * 2. small provider-contract rules for API details models.dev cannot express,
 *    such as Kimi's fixed sampling values and MiniMax output limits.
 *
 * Unknown or ambiguous models deliberately remain in VS Code's automatic state
 * instead of receiving guessed capabilities that could make requests fail.
 */
export function getCopilotByokModelPreset(
  ...
  const catalogPreset = catalogMatch
    ...
    : null;
  const providerPreset = mergePresets(
    kimiContractPreset(context, modelId, modelName, catalogMatch?.modelId),
    miniMaxContractPreset(context, modelId),
  );
  return mergePresets(catalogPreset, providerPreset);
}

function hasFixedKimiSampling(
  context: CopilotByokModelPresetContext,
  modelId: string,
): boolean {
  if (!isKimiProvider(context)) return false;
...
export function mergeCopilotByokModelOptions(
  ...
  // fixed sampling values must override stale/manual values. Other providers
  // retain explicit user overrides over catalog defaults.
  return hasFixedKimiSampling(context, modelId)
